...
def get_access_token_from_gcloud(force=False):
  ...
  if token is None or force:
    token = subprocess.check_output(
        ["/usr/bin/gcloud", "auth", "application-default",
         "print-access-token"],
        text=True,
    ).rstrip()
  return token
...
def build_rows(metric, data):
  ...
  for timeseries in data["timeSeriesData"]:
    ...
    for point_idx in range(len(pointData)):
      ...
      value = timeseries["pointData"][point_idx]["values"][0][
        api_value_type_index]
  ...
  if "linearBuckets" in value_json["bucketOptions"]:
    ...
    exponentialBuckets = {
        "numFiniteBuckets": value_json["bucketOptions"]["exponentialBuckets"][
          "numFiniteBuckets"],
        "growthFactor": round(
            value_json["bucketOptions"]["exponentialBuckets"]["growthFactor"],
            2),
